...
//
//     final searchRetirement = searchRetirementFromJson(jsonString);

import 'dart:convert';

List<SearchRetirement> searchRetirementFromJson(String str) => List<SearchRetirement>.from(json.decode(str).map((x) => SearchRetirement.fromJson(x)));

String searchRetirementToJson(List<SearchRetirement> data) => json.encode(List<dynamic>.from(data.map((x) => x.toJson())));

class SearchRetirement {
  SearchRetirement({
  ...
  factory SearchRetirement.fromJson(Map<String, dynamic> json) => SearchRetirement(
...
enum JomunReference { EMPTY, THE_2021413, JOMUN_REFERENCE_2021413, THE_2022111 }

final jomunReferenceValues = EnumValues({
  "": JomunReference.EMPTY,
  "[본조신설 2021.4.13]": JomunReference.JOMUN_REFERENCE_2021413,
  "[제목개정 2021.4.13]": JomunReference.THE_2021413,
  "[본조신설 2022.1.11]": JomunReference.THE_2022111
});
