...
import '../service/model/retirement_services.dart';

class RetirementDetailScreen extends StatefulWidget {
  const RetirementDetailScreen({Key? key}) : super(key: key);

  @override
  State<RetirementDetailScreen> createState() => _RetirementDetailScreenState();
}

class _RetirementDetailScreenState extends State<RetirementDetailScreen> {
  ...
  static String endPointUrl = API().getRetirementUrl();
  final Uri url = Uri.parse(endPointUrl);

  late Future<RetirementServices> services;
  ...
  Future<RetirementServices> fetchData() async {
    ...
    if (response.statusCode == 200) {
      return RetirementServices.fromJson(json.decode(response.body));
    } else {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Padding(
        ...
        child: FutureBuilder<RetirementServices>(
          future: services,
                                ...
                                Column(
                                  ...
                                  children: [
                                    if (snapshot.data?.empty.sticky.empty[index]
                                      ...
                                      ...[
                                        Text(
                                          "제${snapshot.data?.empty.sticky
                                              .empty[index].fluffy.text}조(${snapshot.data?.empty.sticky.empty[index].ambitious?.cdata})",
                                          style: subTextStyle.copyWith(
                                            fontWeight: FontWeight.w700,
                                            fontSize: 16.0,
                                          ),
                                        ...
                                        if (snapshot.data!.empty.sticky
                                            ...
                                            null) ...[
                                          Text(
                                            "조문 참고 자료 : ${snapshot.data!.empty.sticky.empty[index].cunning?.cdata}",
                                            style: const TextStyle(
                                              color: CupertinoColors
                                                  .darkBackgroundGray,
                                              fontSize: 14.0,
