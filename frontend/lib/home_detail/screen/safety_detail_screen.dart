...
import '../service/model/safety_services.dart';

class SafetyDetailScreen extends StatefulWidget {
  const SafetyDetailScreen({Key? key}) : super(key: key);
...
class _SafetyDetailScreenState extends State<SafetyDetailScreen> {
  ...
  static String endPointUrl = API().getSafetyUrl();
  final Uri url = Uri.parse(endPointUrl);

  late Future<SafetyServices> services;
  ...
  Future<SafetyServices> fetchData() async {
    final response = await http.get(url);

    if (response.statusCode == 200) {
      return SafetyServices.fromJson(json.decode(response.body));
    } else {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Padding(
        ...
        child: FutureBuilder<SafetyServices>(
          future: services,
